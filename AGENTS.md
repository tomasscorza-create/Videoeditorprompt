...
- La Fase U2 integra timeline y edición manual sin crear otra fuente de verdad. El panel derecho `Edición` concentra texto/voz de diálogo, recurso, transformaciones, presets, pistas y keyframes; lee la selección y el cabezal compartidos y solo despacha comandos semánticos existentes. La timeline conserva las operaciones temporales y estructurales —selección, arrastre, división segura, duplicado y borrado— y su cabezal medido es la referencia explícita al crear o reanclar keyframes. Sin medición vigente la UI bloquea la colocación temporal y explica que hace falta renderizar, en vez de inventar segundos.
