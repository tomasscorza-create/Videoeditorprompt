...
- La Fase U3 amplía `Edición` sin exponer capacidades que el render todavía rechaza. Las escenas se reparten en `General`, `Fondo` y `Transición`; los diálogos separan `Texto` de `Voz y gesto`; y los elementos conservan `Ajustes`, `Crear animación` y `Pistas`. Fondo/cámara, transición, personaje hablante, voz, gesto y pausa usan los comandos semánticos existentes y participan del mismo undo/redo. Los ajustes se ordenan en una propiedad por renglón: nombre, valor compacto de hasta dos decimales y, cuando corresponde, navegación y rombo de keyframe en la misma fila. Opacidad se edita como porcentaje numérico. No se ofrece pose base porque el compilador vigente solo admite `neutral` fuera de los turnos.
...
- `scripts/timeline/media-library.mjs`, `timeline-project-repository.mjs` y `timeline-exporter.mjs`: ingest por streaming y hash, persistencia optimista, plan de segmentos y exportación FFmpeg cacheada.
- `src/ui/timeline-v2.ts` y `timeline-v2-api.ts`: adaptador de medios libres dentro de la timeline única, operaciones profesionales, preview compuesto y conexión loopback.
