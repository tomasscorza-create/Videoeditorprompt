...
- La Fase P0 de persistencia portable fue aprobada. P1 separa contratos async para proyectos, recursos, jobs y blobs, conserva `filesystem` como único backend operativo e inyecta sus adaptadores desde el servicio local. No hay drivers PostgreSQL/S3 ni Docker instalados.
- P2 exporta el estado durable a un bundle portable y determinista, verifica esquema, referencias, tamaños y SHA-256, e ignora cachés, temporales, publicaciones regenerables y jobs sin outputs conservados. G2 fue aprobado. P3 levanta PostgreSQL 17.10 y SeaweedFS 4.40 en loopback mediante Compose, con volúmenes, healthchecks y migraciones idempotentes; la aplicación aún usa filesystem y G3 sigue pendiente.
