...
- El paso U2.0 prepara el panel derecho para la edición manual sin adelantar herramientas: elimina la etiqueta redundante «Catálogo local», conserva la biblioteca bajo la página `Recursos`, agrega la página hermana `Edición` y conecta su contexto y host modular con la selección compartida de escena, elemento, diálogo o keyframe.
...
- `src/ui/right-panel.ts`: navegación interna `Recursos / Edición`, persistencia de la página activa y proyección contextual de la selección compartida sobre el host extensible de herramientas manuales.
