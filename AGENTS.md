...
- La evolución UX del Director usa el recorrido guiado `Idea → Base → Video` sin agregar otra fuente de verdad. `Idea` reúne prompt, duración y modelo IA; la cantidad de particiones internas se decide automáticamente y no se expone como opción. Al aceptar, el mismo proveedor/modelo genera exactamente tres preguntas contextuales estructuradas. Cada una ofrece tres opciones específicas y un campo `Otra respuesta` para escribir una alternativa. `Base` muestra únicamente esa segunda capa de personalización, sin storyboard ni otro prompt genérico. El proyecto se crea recién al confirmar las tres respuestas y entonces pasa a `Video`; allí la medición liviana prepara voces, tiempos y preview sin generar un MP4. La exportación final se inicia solo por una acción explícita y muestra bloqueos o progreso real por etapas, sin porcentajes simulados. Un proyecto durable existente abre directamente en `Video`. El proveedor acumula uso de preguntas, reparaciones, segmentos y juez; OpenAI registra tokens cacheados, aplica reintentos transitorios acotados y recibe claves de prompt caching sin texto del usuario. Los planes extensos pueden generarse en bloques equilibrados de hasta tres y `Video` conserva un detalle técnico plegado asociado a la revisión del proyecto.
- La presentación normal trata el proyecto como una única secuencia continua. Las escenas persisten solo como particiones técnicas para conservar render aislado, caché, anclas, compatibilidad y edición estructural, y no se representan en la interfaz. La timeline no dibuja límites, rótulos ni clips por escena: muestra un fondo global y una barra continua por personaje, prop o efecto, con voces y medios libres sobre el mismo tiempo. El Director V2 conserva fondo, cámara estática, disposición, reparto y voces, y la compilación convierte todos los enlaces internos, incluso los históricos, en fundidos deterministas de 0,35 segundos.
- La Fase U2 vincula timeline y Edición mediante una única selección y un único cabezal. Después de un render medido, los cambios visuales y de keyframes pueden previsualizarse en vivo sobre el lienzo sin generar otro MP4: el último MP4 aporta el audio y el reloj, mientras el proyecto actual aporta escenas, fondos precargados, subtítulos y animación. Los keyframes resueltos aparecen como rombos en el clip del personaje o prop correspondiente y abren su ficha de Edición al seleccionarlos; posición y escala usan el mismo evaluador compartido. Cada job conserva además una revisión separada de tiempos para que ese preview sobreviva a una recarga; los cambios que alteran voces o tiempos invalidan la medición y requieren renderizar de nuevo. Una sesión restaurada no sobrescribe un proyecto durable existente hasta que el usuario realiza un cambio.
- La Fase U2 integra timeline y edición manual sin crear otra fuente de verdad. El panel derecho `Edición` concentra texto/voz de diálogo, recurso, transformaciones, presets, pistas y keyframes; para evitar scroll acumulado divide cada elemento en las subpáginas `Ajustes`, `Crear animación` y `Pistas`, abre esta última al seleccionar un keyframe y usa `Texto y voz` para diálogos. `Ajustes` expresa la opacidad como porcentaje con previsualización inmediata; en personajes la persiste como una pista constante compatible con el compilador. Cada propiedad animable visible ofrece navegación `‹ ›` y un rombo `◇/◆`: el primer clic arma una pista con un único punto exactamente en el cabezal, el segundo abre un tramo lineal y los cambios posteriores de la propiedad crean o actualizan el punto de la posición actual. Las articulaciones declaradas por el recurso usan el mismo flujo y la lógica de comandos sigue compartida con lienzo y editor avanzado. El orden visual se presenta como capas numéricas simples: el fondo completo es la capa 0, los personajes y props ocupan 1…N, el número mayor queda encima y cada alta nueva recibe automáticamente la capa superior siguiente; `zIndex` permanece como detalle interno. Visor y exportación normalizan las anclas de autoría al centro del recurso. Al seleccionar un personaje o prop, el visor mide de forma cacheada el canal alfa de su imagen y muestra un único marco transparente ajustado a sus límites visibles, con controles de mouse separados para mover, escalar y rotar; el antiguo contorno punteado de la imagen completa no se dibuja. El arrastre se sigue desde `window` para no perderlo fuera del tirador. Fuera del modo Animación cambian la base y dentro de él escriben keyframes en el cabezal. Una rotación base o animada selecciona automáticamente el compositor Pixi y llega al MP4 incluso con personajes v2; la opacidad base incompatible se informa con su ajuste concreto. Lee la selección y el cabezal compartidos y solo despacha comandos semánticos existentes. La timeline conserva las operaciones temporales y estructurales —selección, arrastre, división segura, duplicado y borrado— sin insertar filas de animación al seleccionar un clip; las pistas y keyframes se eligen en `Edición`. Su cabezal rojo medido es arrastrable y constituye la referencia explícita al crear o reanclar keyframes. Sin medición vigente la UI bloquea la colocación temporal y explica que hace falta renderizar, en vez de inventar segundos. La revisión liviana del proyecto canonicaliza las claves JSON para sobrevivir al roundtrip por PostgreSQL; un render histórico abierto explícitamente sigue marcado como anterior, pero puede reproducirse con el botón principal sin adoptar sus tiempos como edición vigente.
- La Fase U3 amplía `Edición` sin exponer capacidades que el render todavía rechaza. El fondo y la cámara se presentan como propiedades globales del video; los enlaces internos no aparecen como controles. Los diálogos separan `Texto` de `Voz y gesto`; y los elementos conservan `Ajustes`, `Crear animación` y `Pistas`. Personaje hablante, voz, gesto y pausa usan los comandos semánticos existentes y participan del mismo undo/redo. Los ajustes se ordenan en una propiedad por renglón: nombre, valor compacto de hasta dos decimales y, cuando corresponde, navegación y rombo de keyframe en la misma fila. Opacidad se edita como porcentaje numérico. No se ofrece pose base porque el compilador vigente solo admite `neutral` fuera de los turnos.
- La Fase U4 ordena `Crear animación` alrededor de tres tareas explícitas: aplicar animaciones prediseñadas agrupadas, animar con el mouse y crear puntos manuales desde los rombos de `Ajustes`. Los presets exponen intensidad y duración, conservan el desplazamiento exacto del cabezal respecto del ancla semántica y se bloquean antes de sobrescribir una pista manual/personalizada o de producir keyframes fuera de la escena. Una aplicación correcta confirma efecto, tiempo y cantidad de puntos; los conflictos ofrecen acceso directo a `Pistas`; las referencias temporales y los errores se expresan sin IDs, rutas ni códigos internos. `Pistas` cuenta y rotula por separado puntos fuera de escena y referencias pendientes. El visor mantiene el borde de modo animación pero usa un aviso compacto que cubre mover, escalar y rotar.
- Las Fases V0–V8 de variabilidad creativa conservan los cuatro modos (`voiceover`, `solo`, `dialogue`, `visual-with-voiceover`) y el límite de dos personajes simultáneos, sin limitar el reparto total del proyecto. ElevenLabs es el único proveedor TTS admitido por el producto vigente; sintetiza cada turno y FFprobe determina siempre su duración real, mientras `durationWeight` solo distribuye el objetivo editorial. La clave vive exclusivamente en el backend, las voces se importan a un catálogo durable, el audio se normaliza a WAV canónico y se cachea por hash. Las voces Piper fueron retiradas del catálogo y del contrato v2. `ai-video-plan-v1` permanece como compatibilidad histórica y la interfaz normal usa el plan V2. Las recetas se expanden localmente a presets versionados, nunca a código ni keyframes libres de IA.
- El recorte profesional V2 está integrado como pistas de medios libres en la timeline única. Usa fuentes inmutables deduplicadas por SHA-256, tiempo entero a 48.000 ticks por segundo, video a 30 fps, audio a 48 kHz, clips A/V enlazables y desvinculables, corte, trim, movimiento, duplicado, borrado ripple, selección múltiple, snap y undo/redo. El panel derecho suma la página `Archivos`: acepta selección múltiple o arrastre de MP4, MOV, WebM, WAV, MP3, OGG y M4A, los mide con FFprobe, genera un proxy local cuando el codec no es reproducible por el navegador y agrega los clips listos al final de la timeline del proyecto activo. Cada proyecto conserva su documento V2 y su revisión optimista; los originales inmutables permanecen compartidos y deduplicados. Importa medios o adopta un render/medición vigente sin volver a sintetizar voces; comparte el cabezal del Editor y el visor compone la pista visual base como fondo, las capas semánticas y la pista de superposición. `video-project.schema.json` conserva la autoría semántica y `timeline-project-v2.schema.json` mantiene los medios libres existentes sin forzar una migración destructiva.
...
- Existe un runtime probado de **una escena**: 1080 × 1920, 30 fps, dos personajes reutilizables y diferenciados, diálogo ElevenLabs medido por FFprobe, boca híbrida de seis visemas guiada por texto y límites RMS (con fallback RMS), cinco gestos temporizables, prosodia por turno, idle seeded, layouts interpolados, subtítulo por turno, música opcional con ducking y fondo opcional de tres capas con cámara/parallax deterministas, equivalente en PixiJS y MP4 H.264/AAC. Un pipeline padre ya compone varias de esas escenas en un MP4 mediante cortes o fundidos.
...
- No reemplazar PixiJS, FFmpeg, ElevenLabs o el pipeline vigente sin una razón probada y una tarea específica.
...
- TTS, IA, render y storage deben evolucionar hacia contratos reemplazables; no extender los acoplamientos actuales de ElevenLabs o filesystem.
...
- `scripts/stage1/prepare-scene.mjs`: despacho de preparación, FFprobe, RMS, subtítulo y runtime.
...
- `shared/video-template-page.js` (+`.d.ts`): pintor de una página impresa generativa. JavaScript de navegador puro porque lo consumen los dos extremos: el editor y el compositor headless, que corre PixiJS dentro de un Chrome real. Es lo que evita que el render tenga una segunda implementación del efecto.
- `shared/video-template-definition.js` (+`.d.ts`): contrato cerrado de una definición de plantilla, compartido por el editor y `validate-video-project.mjs`.
- `shared/video-template-evaluator.js`: evaluación temporal del efecto por `frameIndex / fps`, sin acumular deltas.
- `public/assets/templates/`: definiciones de plantilla. `procedural-word-match-cut-v1` construye doce páginas por código, sin assets de terceros.
- `pilots/plantilla-pagina-v1/project.json`: piloto de una escena con una plantilla sobre dos personajes; renderiza a MP4 y es determinista.
...
- `docs/FASE_0_RECORTE_PROFESIONAL_V2.md`: contrato congelado, decisiones temporales, invariantes, errores y gate del recorte profesional V2.
- `docs/FASES_2_9_MONTAJE_PROFESIONAL_V2.md`: adopción, ingest, interfaz, preview, exportación incremental, verificación y límites honestos del montaje V2.
- `schema/timeline-project-v2.schema.json` y `schema/timeline-command-v2.schema.json`: contratos cerrados del documento de clips y sus operaciones no destructivas individuales y enlazadas.
- `shared/timeline-clip-core.js` (+`.d.ts`): núcleo puro e inmutable de clips V2 con validación semántica, split/trim/move/duplicate/delete, ripple, enlaces A/V, evaluación de automatización, batch y undo/redo. No importa Node, DOM ni Ajv.
- `scripts/timeline/media-library.mjs`, `timeline-project-repository.mjs` y `timeline-exporter.mjs`: ingest por streaming y hash, proxies de preview, persistencia optimista, plan de segmentos y exportación FFmpeg cacheada.
- `src/ui/timeline-v2.ts`, `timeline-v2-api.ts`, `timeline-media-placement.ts` y `project/media-files-panel.ts`: adaptador de medios libres dentro de la timeline única, biblioteca `Archivos`, drop/picker, colocación A/V enlazada, persistencia por proyecto, preview compuesto y conexión loopback.
- `scripts/timeline/test-timeline-clip-core.mjs`, `test-timeline-media.mjs` y `test-timeline-stress.mjs`: gates de contrato, medios/exportación reales y carga determinista.
...
- `schema/creative-scene-blueprint.schema.json` y `creative-recipe-catalog.schema.json`: contratos aislados V0 para modos flexibles, voz fuera de campo, recetas y secuencias coordinadas; todavía no son contratos de runtime.
- `shared/creative-capabilities.js` (+`.d.ts`): matriz pura y determinista de límites, recursos, animación, recetas y estado real por consumidor.
- `scripts/director/creative-contract.mjs` y `public/assets/catalog/creative-recipes.json`: validación semántica y catálogo inicial de cuatro recetas de escena y cuatro secuencias coordinadas.
- `pilots/director-flexible-v1/`: fixtures V0 válidos e inválidos de los cuatro modos; no se renderizan hasta V1.
- `docs/CONTRATO_VARIABILIDAD_CREATIVA_V0.md` y `PLAN_DIRECTOR_VARIABILIDAD_CREATIVA_V1.md`: contrato vigente, compatibilidad, fases y gates de la ampliación.
- `pilots/runtime-flexible-v1/` y `docs/RUNTIME_FLEXIBLE_V1.md`: proyectos renderizables V1, gate mixto 0/1/2, evidencia determinista y límites todavía no expuestos al Director.
...
- `src/ui/timeline.ts`: timeline audiovisual, transporte, zoom, snap, cabezal arrastrable, atajos y despacho al medio activo; selecciona clips sin desplegar inspectores o pistas derivadas y nunca estima duración como si fuera medida.
- `src/ui/project/composition.ts`: proyección visual estática del proyecto y catálogo; no evalúa audio, boca ni tiempos.
- `src/ui/project/layers.ts`: traducción única entre el `zIndex` interno y las capas humanas 1…N, reordenamiento semántico y asignación de la siguiente capa al agregar elementos.
...
- `src/ui/right-panel.ts`: navegación interna `Recursos / Edición`, persistencia de la página activa y descripciones contextuales de la selección compartida.
- `src/ui/project/editing-panel.ts`: mando manual contextual conectado al mismo `ProjectStore`, selección, medición y cabezal que usan lienzo y timeline; no mantiene pistas ni valores paralelos.
...
npm run timeline:test-clip-core # núcleo profesional V2: clips no destructivos, tiempo entero y undo/redo
npm run timeline:test-media # ingest, deduplicación, persistencia y exportación FFmpeg real con caché
npm run timeline:test-stress # 500 clips, 100 cortes, historial acotado y determinismo
...
npm run director:test-creative-contract # modos 0/1/2, recetas, secuencias y matriz honesta V0
npm run stage3a:flexible-pipeline # voz fuera de campo + prop, dos MP4 deterministas
npm run stage3a:flexible-solo-pipeline # monólogo de un personaje, dos MP4 deterministas
...
npm run worker:test-portability # rutas del runtime TTS Windows/Linux
...
Para otros trabajos, invocar `scripts/stage1/pipeline.mjs` con `--job-id` y las raíces necesarias. Configurar ElevenLabs con `ELEVENLABS_API_KEY` o `ELEVENLABS_API_KEY_FILE`; `LOCAL_VIDEO_TTS_ROOT`/`--tts-root` conserva únicamente caché y fuentes locales.
...
- El Director usa un catálogo de tipos/capacidades cerrados, ampliable mediante fichas locales validadas y consultado mediante shortlist determinista. Permite elegir el modelo al iniciar y formula tres preguntas aclaratorias estructuradas antes de crear el proyecto; todavía no usa búsqueda semántica por embeddings ni repara automáticamente cualquier plan rechazado. Sí combina cinco plantillas narrativas, compara hasta tres variantes y repara rechazos por presupuesto de palabras.
- El runtime, el compilador y el proyecto renderizado ya consumen `voiceover`, `solo` y estructuras visuales narradas con 0–2 personajes. El Editor todavía no expone todos esos controles y el Director actual continúa generando el plan V1 rígido de dos personajes; no debe recibir los nuevos modos, recetas o secuencias hasta completar sus gates V2–V4.
- La biblioteca V1 registra descriptores, importa fondos estáticos JPG/PNG y fondos animados GIF/MP4 en una raíz durable externa al repositorio. Los animados se normalizan a MP4 H.264 vertical 1080 × 1920, 30 fps, sin audio, con póster y loop determinista; visor y exportación los sincronizan desde `frameIndex / fps`. También guarda personajes creados desde plantillas o piezas geométricas semánticas. `public/assets/library` es solo su publicación regenerable y el arranque migra sin borrar la ubicación legacy; todavía no elimina recursos, importa rigs externos ni genera assets con IA.
- El Director acepta tono, duración objetivo y cantidad de escenas como restricciones del JSON Schema; siguen siendo objetivos editoriales y la duración real solo existe después de TTS/FFprobe.
...
- La timeline muestra capas estructurales y permite selección/navegación. El único corte disponible es `split-dialogue-turn`, con la tecla `B` sobre el turno que ocupa el cabezal (dividir la escena, que es estructural, pasó a `Ctrl+B`): parte un turno en dos por una frontera de palabra, porque cada turno es una síntesis entera de ElevenLabs y cortar significa producir dos enunciados que se vuelven a medir, nunca recortar un WAV. El punto de corte sale del cabezal por el mismo prorrateo que ubica un ancla de palabra, así que exige medición vigente y hereda su precisión; la pausa del turno viaja a la segunda mitad y el gesto acompaña a su palabra. El corte invalida la medición, pero recuperarla no necesita un render: `scripts/stage3a/measure-project.mjs` corre solo compilación y preparación (ElevenLabs + FFprobe) y arma la línea de tiempo con las mismas funciones que el render, así que produce una salida idéntica. El servicio local lo expone en `POST /api/measurements` y la timeline vuelve a estado medido sola después de cada corte. `sceneRenderDurationSeconds` en `shared/scene-evaluator.js` es lo que lo hace posible: la duración de una escena es una función pura de su audio y sus fps. Un elemento visual puede declarar una **ventana de visibilidad** (`visibility`, opcional y aditiva) anclada con el mismo vocabulario que los keyframes, así cortar o editar un diálogo la mueve con su turno. Fuera de la ventana el evaluador devuelve opacidad 0, que es el único canal que los dos compositores ya leen por frame; como el overlay de FFmpeg no acepta alfa por personaje, un elemento con ventana enruta su escena a PixiJS igual que un prop o un rig v3. Verificado contra dos renders del piloto: PSNR 33 dB dentro de la ventana y 19 dB fuera. **Subir `SCENE_CACHE_VERSION` en `project-pipeline.mjs` cuando cambie cómo se renderiza una escena**: la clave de caché mira config, catálogo y assets, así que un cambio de comportamiento del render es invisible para ella y serviría un MP4 viejo. Arrastrar los bordes de un clip, los keyframes libres y la mezcla editable de música/SFX siguen fuera de alcance. El runtime ya admite una pista musical opcional por video con loop y ducking fijo; todavía no hay clips musicales editables ni SFX puntuales.
...
- 3A.1 solo compila escenas con exactamente dos personajes, al menos dos turnos, pose inicial neutral, ancla central, rotación 0 y opacidad 1. Texto e imágenes se rechazan explícitamente hasta que el runtime pueda representarlos.
- Una escena admite elementos de tipo `template`: efectos prearmados del catálogo de autoría que cubren el cuadro completo y repiten su ciclo mientras dura la escena. Se agregan arrastrándolos al visor, exponen solo los campos que declara su definición (hoy una palabra), participan del orden de capas y del undo/redo, y **fuerzan el compositor PixiJS** porque se dibujan por código en cada frame. No admiten pistas de keyframes. El Director todavía no puede proponerlos: su esquema no emite ese tipo de elemento.
...
- El piloto 2D histórico reutiliza el mismo rig y modelo Piper; el producto vigente reemplazó esas voces por recursos ElevenLabs importados.
...
- La boca usa seis visemas derivados de grafemas españoles y acotados por actividad RMS, con fallback RMS. Todavía no se consume alineación fonética de ElevenLabs y la mejora técnica no equivale a aprobación visual.
...
- `prepare-scene.mjs` conserva el lector histórico v1 únicamente para fixtures antiguos. El diálogo v2 usa `tts-voice.mjs` y admite exclusivamente ElevenLabs, que produce WAV PCM canónico antes de FFprobe, análisis y composición.
