...
- Las Fases V0–V8 de variabilidad creativa conservan los cuatro modos (`voiceover`, `solo`, `dialogue`, `visual-with-voiceover`) y el límite de dos personajes simultáneos, sin limitar el reparto total del proyecto. ElevenLabs es el único proveedor TTS admitido por el producto vigente; sintetiza cada turno y FFprobe determina siempre su duración real. `durationWeight` solo distribuye el objetivo editorial. Las voces Piper fueron retiradas del catálogo y del contrato v2. `ai-video-plan-v1` permanece como compatibilidad histórica y la interfaz normal usa el plan V2. Las recetas se expanden localmente a presets versionados, nunca a código ni keyframes libres de IA.
...
- `prepare-scene.mjs` conserva el lector histórico v1 solo para fixtures antiguos. El diálogo v2 usa `tts-voice.mjs` y admite exclusivamente ElevenLabs, que produce WAV PCM canónico antes de FFprobe, análisis y composición.
