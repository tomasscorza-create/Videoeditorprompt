...
- El recorte profesional V2 está operativo como espacio `Montaje` para proyectos nuevos. Usa fuentes inmutables deduplicadas por SHA-256, tiempo entero a 48.000 ticks por segundo, video a 30 fps, audio a 48 kHz, clips A/V enlazados, corte, trim, movimiento, duplicado, borrado ripple, selección múltiple, snap y undo/redo. Importa medios o adopta un render/medición vigente sin volver a sintetizar voces; el preview comparte un único reloj y la exportación FFmpeg solo ocurre por petición, reutilizando segmentos y outputs por hash. `video-project.schema.json` conserva la autoría semántica de personajes, props y plantillas; `timeline-project-v2.schema.json` es la fuente durable del montaje posterior, sin migración obligatoria de proyectos anteriores.
...
- `src/ui/timeline-v2.ts` y `timeline-v2-api.ts`: espacio `Montaje`, preview A/V de reloj único, operaciones profesionales y conexión loopback.
