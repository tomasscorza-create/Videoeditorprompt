...
- La Fase 0 del recorte profesional V2 congela un modelo no destructivo basado en fuentes inmutables, clips editables, tiempo entero a 48.000 ticks por segundo, video a 30 fps y audio a 48 kHz. La Fase 1 aporta un núcleo puro y aislado con schemas propios, validación semántica, colisiones explícitas, automatización en tiempo de fuente, comandos `split/trim/move/duplicate/delete`, lotes atómicos y undo/redo. Todavía no reemplaza `video-project.schema.json`, no está conectado a la interfaz y no constituye otra fuente de verdad durable; la migración corresponde a la Fase 2.
...
- `schema/timeline-project-v2.schema.json` y `schema/timeline-command-v2.schema.json`: contratos aislados del documento de clips y sus cinco operaciones no destructivas.
- `shared/timeline-clip-core.js` (+`.d.ts`): núcleo puro e inmutable de clips V2 con validación semántica, split/trim/move/duplicate/delete, batch y undo/redo. No importa Node, DOM ni Ajv.
- `scripts/timeline/test-timeline-clip-core.mjs`: gate contractual de las Fases 0–1; cubre frames, muestras, fuentes, automatización, colisiones, atomicidad y determinismo.
