...
- Las Fases V0–V8 de variabilidad creativa conservan los cuatro modos (`voiceover`, `solo`, `dialogue`, `visual-with-voiceover`) y el límite de dos personajes simultáneos, sin limitar el reparto total del proyecto. Piper y FFprobe siguen determinando la duración real; `durationWeight` solo distribuye el objetivo editorial. `ai-video-plan-v1` permanece como compatibilidad histórica y la interfaz normal usa el plan V2. Las recetas se expanden localmente a presets versionados, nunca a código ni keyframes libres de IA.
