...
import { ffprobe } from '../stage1/common.mjs';
...
export async function createTimelineMediaLibrary(options = {}) {
  ...
  async function importStream(readable, input = {}) {
    ...
    try {
      ...
      const entry = buildEntry({ probe, contentHash, bytes, mimeType, extension, fileName: input.fileName });
  ...
  return { storageRoot, list, get, open, importStream, importFile };
}
