...
try {
  ...
  assert.equal((await library.list()).length, 11);
  ...
  assert.equal((await restored.list()).length, 16);
  ...
  assert.equal(JSON.parse(readFileSync(restored.catalogPath, 'utf8')).entries.length, 16);
  assert.equal(existsSync(path.join(
    publishRoot,
    'backgrounds',
    importedBackground.resource.id,
  ...
  assert.equal((await migrated.list()).length, 16);
