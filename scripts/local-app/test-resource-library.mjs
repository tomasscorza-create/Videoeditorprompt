...
try {
  ...
  assert.equal((await restored.list()).length, 15);
  ...
  assert.equal(JSON.parse(readFileSync(restored.catalogPath, 'utf8')).entries.length, 15);
  assert.equal(existsSync(path.join(
    publishRoot,
    'backgrounds',
    importedBackground.resource.id,
  ...
  assert.equal((await migrated.list()).length, 15);
