...
import { createProjectEditor } from '../../shared/project-editor.js';
...
function verdict(project) {
  ...
  return { validator, editor, compiler };
}

assert.deepEqual(verdict(structuredClone(base)), { validator: true, editor: true, compiler: true });

const duplicateScene = structuredClone(base);
duplicateScene.scenes[1].id = duplicateScene.scenes[0].id;
assert.deepEqual(verdict(duplicateScene), { validator: false, editor: false, compiler: false });

const futureTransform = structuredClone(base);
futureTransform.scenes[0].elements[0].transform.rotationDegrees = 10;
assert.deepEqual(verdict(futureTransform), { validator: true, editor: false, compiler: false });

const missingTransition = structuredClone(base);
delete missingTransition.scenes[0].transitionToNext;
assert.deepEqual(verdict(missingTransition), { validator: false, editor: false, compiler: false });
